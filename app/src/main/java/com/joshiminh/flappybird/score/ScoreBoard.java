  ...
  public ScoreBoard() {
    try (Connection connection = Database.getConnection();
        PreparedStatement ps = connection.prepareStatement(
            "SELECT PlayerName, MAX(Score) AS MaxScore FROM single_player_scores GROUP BY "
            + "PlayerName ORDER BY MaxScore DESC");
      ...
      var star = new ImageIcon(new ImageIcon(ResourceUtil.getResource("/images/star.png"))
              .getImage()
              .getScaledInstance(45, 45, Image.SCALE_DEFAULT));

      JOptionPane.showMessageDialog(
          null, new JScrollPane(table), "Player Scores", JOptionPane.INFORMATION_MESSAGE, star);

    } catch (SQLException e) {
      JOptionPane.showMessageDialog(
          null, "Database error: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    }
  }
